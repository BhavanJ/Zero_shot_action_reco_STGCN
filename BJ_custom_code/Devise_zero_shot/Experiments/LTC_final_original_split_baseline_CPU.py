...
# FEATURE_DIR = '/home/bjasani/Desktop/CMU_HW/VLR_project/STGN/st-gcn/work_dir/NTU-RGB-D_zero_shot_nearest_neigh/xview/ST_GCN_exp1_features/'
# SKELETON_DIR = '/home/bjasani/Desktop/CMU_HW/VLR_project/STGN/st-gcn/data/NTU-RGB-D_zero_shot_nearest_neigh/xview/'
UNSEEN_CLASSES = [3,8,11,16,51] # For nearest neighbhours...classes go from 0 to 59 here 
...
#                                     relation_network =  2* size of visualfeat, some hidden size ....evnetaully this networks output is 1
	attribute_network = AttributeNetwork(DIM_LANGAUGE,int(DIM_STGCN/2),DIM_STGCN)
	relation_network = RelationNetwork(2*DIM_STGCN,int(DIM_STGCN/2))
	...
	for episode in range(EPISODE):
		...
		batch_features = Variable(batch_features).cuda(GPU).float()  if use_cuda else Variable(batch_features).float()# 32*1024
		sample_features = attribute_network(Variable(sample_attributes).cuda(GPU)) if use_cuda else attribute_network(Variable(sample_attributes))#k*312
...
#DTODO: CHANGE 4096        
		...
		# loss
		mse = nn.MSELoss().cuda(GPU) if use_cuda else nn.MSELoss()
		one_hot_labels = Variable(torch.zeros(BATCH_SIZE, class_num).scatter_(1, re_batch_labels.view(-1,1), 1)).cuda(GPU) if use_cuda else Variable(torch.zeros(BATCH_SIZE, class_num).scatter_(1, re_batch_labels.view(-1,1), 1))
		...
		if (episode+1)%2000 == 0:
			...
			def compute_accuracy(test_features,test_label,test_id,test_attributes, top5=True):
				
				test_data = TensorDataset(test_features,test_label)
				test_batch = 32
				test_loader = DataLoader(test_data,batch_size=test_batch,shuffle=False)
				total_rewards = 0
				top5_total_rewards = 0.0
				top5_test_accuracy = 0.0
				# fetch attributes
				# pdb.set_trace()
				...
				for batch_features,batch_labels in test_loader:
					...
					batch_features = Variable(batch_features).cuda(GPU).float() if use_cuda else Variable(batch_features).float() # 32*1024 
					sample_features = attribute_network(Variable(sample_attributes).cuda(GPU).float()) if use_cuda else attribute_network(Variable(sample_attributes).float())
...
#   batch_labels are from 0 to 59 ....in case of useen they are only the 5 values 
#	re_batch_labels are the true labels ....shape is just torch.Size([32]).....which are mapped to 0 to 4
#   predict_labels are the predictions ....shape is just torch.Size([32]).....based on relations and so these go in possile classes
#   rewards is the accuracy thing

					# pdb.set_trace()

					_,predict_labels = torch.max(relations.data,1)

					_,top5_predict_labels = torch.topk(relations.data,k=5)

# predict_labels.........shape is 32
# top5_predict_labels....shape is 32x5

					rewards = [1 if predict_labels[j]==re_batch_labels[j] else 0 for j in range(batch_size)]

					for jjj in range(batch_size):
						if re_batch_labels[jjj] in top5_predict_labels[jjj]:
							top5_total_rewards += 1.0


					total_rewards += np.sum(rewards)
					

				test_accuracy = total_rewards/1.0/test_size*100.0
				top5_test_accuracy = top5_total_rewards/1.0/test_size*100.0
				# pdb.set_trace()


				return test_accuracy,top5_test_accuracy
			
#DTODO: CHANGE np.arrange(50)....to 60...NUMPY ARRAY [0....59]
			
			zsl_accuracy,_ = compute_accuracy(test_features,test_label,test_id,test_attributes,top5=False)
			gzsl_unseen_accuracy,top5_gzsl_unseen_accuracy = compute_accuracy(test_features,test_label,np.arange(NO_CLASSES),attributes,top5=True)
			gzsl_seen_accuracy,top5_gzsl_seen_accuracy = compute_accuracy(test_seen_features,test_seen_label,np.arange(NO_CLASSES),attributes,top5=True)
			
			all_test_features = torch.cat((test_features,test_seen_features),dim=0)
			all_test_label = torch.cat((test_label,test_seen_label.unsqueeze(1)),dim=0)
			all_test_accuracy,top5_all_test_accuracy = compute_accuracy(all_test_features,all_test_label,np.arange(NO_CLASSES),attributes,top5=True)

			print('###############')
			print('All videos all classes       : Top1: {}  Top5: {} '.format(all_test_accuracy,top5_all_test_accuracy))
			print('Unseen videos all classes    : Top1: {}  Top5: {} '.format(gzsl_unseen_accuracy,top5_gzsl_unseen_accuracy))
			print('Unseen videos unseen classes : Top1: {}  Top5: -  '.format(zsl_accuracy))
			print('Seen videos all classes      : Top1: {}  Top5: {} '.format(gzsl_seen_accuracy,top5_gzsl_seen_accuracy))
			print('################\n')

			# H = 2 * gzsl_seen_accuracy * gzsl_unseen_accuracy / (gzsl_unseen_accuracy + gzsl_seen_accuracy)
			# print('zsl:', zsl_accuracy)
			# print('gzsl: seen=%.4f, unseen=%.4f, h=%.4f' % (gzsl_seen_accuracy, gzsl_unseen_accuracy, H))
			# print('All test videos,all classes:')

			if zsl_accuracy > last_accuracy:

				# save networks
				torch.save(attribute_network.state_dict(),"./models_LTC/zsl_attribute_network_original_split_baseline.pkl")
				torch.save(relation_network.state_dict(),"./models_LTC/zsl_relation_network_original_split_baseline.pkl")
