...
SAVE_LOCATION = './work_dir/NTU_pretrained_ext_feat/pretrained_features/'
SAVE_NAME = '_pretrained_feat'

####DETAILS: THESE JUST LOADS PRETRAINED WEIGHTS FROM NTU AND GENERATES 256D FEATURES FOR ALL THE CLASSES
...
def get_parser():
    ...
    parser.add_argument(
        '--work-dir',
        default='./work_dir/NTU_pretrained_ext_feat',
        help='the work folder for storing results')
    parser.add_argument(
        '--config',
        default='./config/st_gcn/nturgbd-cross-view_pretrained_feat_ext/test.yaml',
...
##################################################
                ...
                features_array = np.append(features_array,feature_256_batch,axis=0)
                labels_array = np.append(labels_array,label_batch,axis=0)
            ...
            print('Final results on  ' + ln + '  ALL class accuracy:' + str(correct_all/total_all*100.0) )
##################################################
...
if __name__ == '__main__':
    ...
    if p.config is not None:
        print('Reading the YAML file at: '+ p.config)
